...
1. **Palette.** Brief says "warm neutral surfaces, gentle browns, muted greens." Wireframes ship cool grayscale. Which is the truth? This system follows the wireframes; happy to fork a "warm cozy" variant on top.
2. **Logo.** The auth screens use a 🍳 emoji on a gray square. If you have a real wordmark / mark, drop it into `assets/` and we'll re-render the auth/login cards.
3. **Typeface.** Brand display face is **Fraunces** (variable, files supplied). Body is the system sans stack. Confirm the body face — happy to bring in a paired sans (e.g. Söhne, Inter Tight) if you want one.
4. **Desktop polish.** Brief calls it "a real second-class concern" — is there a target desktop layout (centered single column? sidebar nav?) you want recreated, or should the design system stay mobile-only for now?
5. **Hover states.** Mobile-first wireframes don't define them. Confirm the recommended `gray-100 → gray-200` hover tint is fine.
