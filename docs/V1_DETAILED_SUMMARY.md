...
- **Prisma + Postgres:** `prisma/schema.postgres.node.prisma` (JS client) drives the Next runtime locally and in Cloud Run; `prisma/schema.postgres.prisma` (Python client) drives FastAPI and Cloud SQL migrations. Both schemas model the domain with future multi-family flexibility even though V1 seeds a single `FamilySpace`. The seed script (`prisma/seed.ts`) hashes an initial master key and loads the curated tag catalog.
