...
- [authStore.ts](authStore.ts) — the client-side access-token store. Holds the access token **in memory only** (never localStorage/cookie) and registers it with `apiClient` via `setAccessTokenProvider` + `setRefreshHooks`. This is the piece that makes the post-cutover auth flow work: SSR resolves the user from cookies, then the client mints a token here and every subsequent `apiClient` call picks it up.
...
- [recipes.ts](recipes.ts) — `getRecipes`: the browse filter/search query (title, course, tags, difficulty, time, servings, up to 5 ingredient keywords). Called by the SSR page [src/app/(app)/recipes/page.tsx](../app/%28app%29/recipes/page.tsx), not by an API route — `/api/recipes` was deleted in #231.
...
**No raw SQL.** Use Prisma. If a query is unwieldy, build it with relation includes rather than dropping to `$queryRaw`. (Family scoping and the storage-key rule apply here too — they're in the root [CLAUDE.md](../../CLAUDE.md) because they span runtimes.)
