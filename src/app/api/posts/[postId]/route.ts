...
const photoOrderSchema = z.array(photoOrderEntrySchema).max(MAX_PHOTO_COUNT).optional();
...
function extractChangeNote(value: unknown): { changeNote?: string; error?: string } {
...
export async function GET(
  ...
  try {
    ...
    const rawCommentLimit =
      commentLimitParam !== null ? Number.parseInt(commentLimitParam, 10) : undefined;
    const rawCommentOffset =
      commentOffsetParam !== null ? Number.parseInt(commentOffsetParam, 10) : undefined;
    ...
    const rawCookedLimit =
      cookedLimitParam !== null ? Number.parseInt(cookedLimitParam, 10) : undefined;
    const rawCookedOffset =
      cookedOffsetParam !== null ? Number.parseInt(cookedOffsetParam, 10) : undefined;

    const post = await getPostDetail(postId, user.familySpaceId, user.id, {
      commentLimit:
        typeof rawCommentLimit === 'number' && Number.isFinite(rawCommentLimit)
          ? rawCommentLimit
          : undefined,
      commentOffset:
        typeof rawCommentOffset === 'number' && Number.isFinite(rawCommentOffset)
          ? rawCommentOffset
          : undefined,
      cookedLimit:
        typeof rawCookedLimit === 'number' && Number.isFinite(rawCookedLimit)
          ...
          : undefined,
      cookedOffset:
        typeof rawCookedOffset === 'number' && Number.isFinite(rawCookedOffset)
          ? rawCookedOffset
          : undefined,
    });
...
export async function PUT(
  ...
  try {
      ...
      return NextResponse.json(
        {
          error: {
            ...
            message: photoOrderResult.error.errors[0]?.message ?? 'Invalid photo order',
          },
        },
        { status: 400 }
      );
    ...
    const effectivePhotoOrder: PhotoOrderEntry[] =
      requestedPhotoOrder ?? [
        ...existingPost.photos.map((photo) => ({ type: 'existing' as const, id: photo.id })),
        ...files.map((_, index) => ({ type: 'new' as const, fileIndex: index })),
      ];
    ...
    const newEntryCount = effectivePhotoOrder.filter((entry) => entry.type === 'new').length;
    ...
    const existingPhotoMap = new Map(existingPost.photos.map((photo) => [photo.id, photo]));
    ...
    for (const entry of effectivePhotoOrder) {
      if (entry.type === 'existing') {
        ...
        orderedPhotoEntries.push({ type: 'existing', id: photo.id, url: photo.url });
      } else {
        ...
        orderedPhotoEntries.push({ type: 'new', fileIndex: entry.fileIndex, url: '' });
      }
    }
    ...
    const keepExistingIds = orderedPhotoEntries
      .filter((entry): entry is Exclude<ResolvedPhotoEntry, { type: 'new' }> => entry.type === 'existing')
      .map((entry) => entry.id);
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (recipeInput) {
        const recipePayload = {
          ...
          courses: normalizedRecipeCourses.length ? JSON.stringify(normalizedRecipeCourses) : null,
    ...
    const updatedPost = await getPostDetail(postId, user!.familySpaceId, user!.id);
        ...
        return NextResponse.json(
          {
            error: {
              code: 'PHOTO_ORDER_MISMATCH',
              message: 'Unable to match uploaded photos to the requested order',
            },
          },
          { status: 400 }
        );
