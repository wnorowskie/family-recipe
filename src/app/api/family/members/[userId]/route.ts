...
export const DELETE = withAuth(
  async (request, user, context?: { params: Promise<{ userId: string }> }) => {
    const resolvedParams = await context!.params;
    try {
      const validation = parseRouteParams(resolvedParams, userIdParamSchema);
      ...
      logError('family.members.remove.error', error, {
