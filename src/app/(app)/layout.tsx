import { headers } from 'next/headers';
import { redirect } from 'next/navigation';
import AuthBootstrap from '@/components/AuthBootstrap';
import LogoutButton from '@/components/LogoutButton';
import BottomNav from '@/components/navigation/BottomNav';
import NotificationBell from '@/components/navigation/NotificationBell';
import { fetchSessionUser } from '@/lib/auth/bootstrapFromCookies';

export default async function AppLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  // SSR fetches the user via /v1/auth/session — non-rotating, replay-safe.
  // The chain only advances when the client-side <AuthBootstrap> calls
  // /api/auth/bootstrap on mount, which is a route handler that can
  // propagate the rotated cookies back to the browser. See issue #173.
  const headerStore = await headers();
  const cookieHeader = headerStore.get('cookie');
  const result = await fetchSessionUser(cookieHeader);

  if (!result.ok) {
    // Append _se=1 so the middleware doesn't redirect an authenticated user
    // back to /timeline, which would create an infinite redirect loop when
    // the upstream session call keeps failing (network error, misconfiguration).
    redirect('/login?_se=1');
  }

  // Defer the unread-count fetch to <NotificationBell>'s client-side effect,
  // which calls /v1/notifications/unread-count via apiClient with the
  // in-memory access token. The badge briefly shows 0 before the first tick
  // replaces it — acceptable trade-off vs. minting a server-side access token
  // (which would rotate the refresh chain on every layout render, defeating
  // the /session split from #173).

  return (
    <AuthBootstrap>
      <div className="min-h-screen bg-gray-50 pb-24">
        <header className="sticky top-0 z-30 border-b border-gray-200 bg-white/95 backdrop-blur">
          <div className="mx-auto flex max-w-3xl lg:max-w-2xl items-center justify-between px-4 py-4 gap-4">
            <div>
              <h1 className="text-xl font-bold text-gray-900">
                Wnorowski Family Recipe
              </h1>
              <p className="text-sm text-gray-500">
                Hi {result.user.name.split(' ')[0]}!
              </p>
            </div>
            <div className="flex items-center gap-3">
              <NotificationBell initialCount={0} />
              <LogoutButton />
            </div>
          </div>
        </header>
        <main className="mx-auto mt-4 max-w-3xl lg:max-w-2xl px-4 pb-10">
          {children}
        </main>
        <BottomNav />
      </div>
    </AuthBootstrap>
  );
}
