...
const mockPrismaFindFirst = prisma.familyMembership
  .findFirst as jest.MockedFunction<typeof prisma.familyMembership.findFirst>;
...
describe('DELETE /api/family/members/[userId]', () => {
  ...
  describe('Authentication', () => {
    it('requires authentication', async () => {
      ...
      const request = new NextRequest(
        'http://localhost/api/family/members/user_target',
        {
          method: 'DELETE',
        }
      );
  ...
  describe('Validation', () => {
    it('returns 400 for invalid user ID format', async () => {
      const request = new NextRequest(
        'http://localhost/api/family/members/invalid-id',
        {
          method: 'DELETE',
        }
      );
  ...
  describe('Member Existence', () => {
    it('returns 404 for non-existent user', async () => {
      ...
      const request = new NextRequest(
        'http://localhost/api/family/members/clq1234567890',
        {
          method: 'DELETE',
        }
      );
      ...
      const response = await DELETE(request, {
        params: Promise.resolve({ userId: 'clq1234567890' }),
      });
      const data = await parseResponseJSON(response);
  ...
  describe('Permissions', () => {
    ...
    it('prevents user from removing themselves', async () => {
      ...
      const request = new NextRequest(
        `http://localhost/api/family/members/${mockOwner.id}`,
        {
          method: 'DELETE',
        }
      );
      ...
      const response = await DELETE(request, {
        params: Promise.resolve({ userId: mockOwner.id }),
      });
      const data = await parseResponseJSON(response);
